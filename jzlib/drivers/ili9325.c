...
static int col = 0;
static int row = 0;
static int vscroll_base = 0;
static int __backlight = 32 * 2 + 13; /* PC13/PWM3 -- LCD backlight (A-41) */
extern int bg_color;
extern int fg_color;
...
extern void ILI9325_Clear(int height);

void ILI9325_Init(int reset, int cs, int backlight)
{
	ILI93XX_Init(reset, cs);
	// VCI=2.8V
	//************* Reset LCD Driver ****************//
	ILI93XX_Reset(1);
	delay_ms(1);
	...
	col = row = 0;
	ILI9325_Clear(0);
	if (backlight) __backlight = backlight;
	if (__backlight > 0) {
		__gpio_as_output(__backlight);
		__gpio_set_pin(__backlight);
	} else if (__backlight < 0) {
		__gpio_as_output(- __backlight);
		__gpio_clear_pin(- __backlight);
	}
}
...
#if 0
void ILI9325_Hex(unsigned int  d)
{
	unsigned char c[9];
	char i;
	for(i = 0; i < 8; i++)
...
static int __ili9325_putc (char c, FILE *fp)
{
	char buf[2];
	buf[0] = c;
	buf[1] = 0;
...
FILE *fdevopen_ili9325(FILE *fp, int reset, int cs, int backlight)
{
	ILI9325_Init(reset, cs, backlight);
//	ILI9325_Text("ILI9325 initialized\n--fp ");
//	ILI9325_Hex(fp);

	if (fp) {
		fdev_setup_stream(fp, __ili9325_putc, NULL, _FDEV_SETUP_WRITE);
	} else {
		fp = fdevopen(__ili9325_putc, NULL);
	}
//	ILI9325_Text("end fdevopen\n");
	return fp;
}
