...
extern void ILI9326_Clear(int height);

void ILI9326_Init(int reset, int cs, int backlight)
{
...
static int __ili9326_putc (char c, FILE *fp)
{
	char buf[2];
	buf[0] = c;
	buf[1] = 0;
...
FILE *fdevopen_ili9326(FILE *fp, int reset, int cs, int backlight)
{
	ILI9326_Init(reset, cs, backlight);

	if (fp) {
		fdev_setup_stream(fp, __ili9326_putc, NULL, _FDEV_SETUP_WRITE);
	} else {
		fp = fdevopen(__ili9326_putc, NULL);
	}
	return fp;
}
